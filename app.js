...
const choiceObj = {
    3: threeLetterWordsObj,
    4: fourLetterWordsObj,
    5: fiveLetterWordsObj,
    6: sixLetterWordsObj
}
const answerBox = {
    0: answerBox0,
...
function buildDictionary(dictWords,dictImages) {
    dictionary.innerHTML= ""
    for ( let i = 0; i < dictWords.length; i++ ) {
        dictionary.innerHTML += `<div class="dictionary-image">${dictWords[i]} <img src="${dictImages[i]}"></div>`
    }
}
function renderGame(type){
    ...
    gameArr = Object.keys(choiceObj[gameType])
    buildDictionary(Object.keys(choiceObj[gameType]),Object.values(choiceObj[gameType]))
    start(gameArr)
}
function start(arr) {
...
function checkGuess() {
    ...
    if (gameActive) {
        if (answerCount !== gameType) {
            ...
            let currentImage = choiceObj[gameType][thisGuess]
...
function playerWins() {
    ...
    let correctImage = choiceObj[gameType][word]
